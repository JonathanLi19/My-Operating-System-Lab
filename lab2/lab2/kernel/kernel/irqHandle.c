...
void KeyboardHandle(struct TrapFrame *tf){
	...
	if(code == 0xe){ // 退格符
		...
		if(displayCol>0&&displayCol>tail){
			displayCol--;
			uint16_t data = 0 | (0x0c << 8);                                 //why ? (0x0c << 8) means the color is red
		...
		if(character!=0){
			putChar(character);//put char into serial                        why?                if you input a number it's printed in the serial
...
void syscallGetChar(struct TrapFrame *tf){
	...
	while(c == 0){
		...
		putChar(c);        //it's like you input a number, and it is printed on the screen, but it's already printed in keyboard_handle? -- yeah! print twice                                 
		disableInterrupt();
	}
	tf->eax=c;
